...
{
    int64_t mul;
#ifdef OVERFLOW_SAFTEY
    int overflow = __builtin_smulll_overflow(a, b, &mul);
    if (overflow)
    {
        int64_t d = 0, mp2 = m >> 1;
        int64_t i;
...
int64_t sum_binomial_mod(int64_t k, int64_t N, int64_t m)
{
    ...
    // Trying just using a % prime == 0 didn't seem to work
    // TODO: Reinvestigate (was slightly slower too)
    ...
    for (int i = 0; i < factor_count; i++)
    {
        ...
        // memcpy seems to be slower
        // memcpy(closest_multiple_b, prime_factors, factor_count * sizeof(int64_t));
    ...
    for (int64_t j = 1; j <= k; j++)
    {
        ...
        for (int i = 0; i < factor_count; i++)
        {
            ...
            if (closest_multiple_a[i] == a)
            {                          // If a is equal to nearest multliple, prime is a factor of a
                a_complement /= prime; // Factor out prime
                R[i] *= prime;         // Add power of prime to corresponding R
                while (a_complement % prime == 0)
                { // Repeat for all instances of the prime
                    R[i] *= prime;
                    a_complement /= prime;
                }
